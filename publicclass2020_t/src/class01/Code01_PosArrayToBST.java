...
public class Code01_PosArrayToBST {
	...
	//my code
	public static Node myPosToBST(int[] arr, int start, int end){
		if(start > end){
			return null;
		}
		...
		head.right = myPosToBST(arr, i+1, end - 1);
		head.left = myPosToBST(arr, start, i);
		return head;
	}
	//
	...
	public static void main(String[] args) {
		...
		//my function result
		Node myHead = myPosToBST(pos, 0, pos.length - 1);
...
//			}
			if (!isSameValueStructure(head, myPosToBST(pos, 0, pos.length - 1))){
				System.out.println("Oops!");
				break;
			}
		}
